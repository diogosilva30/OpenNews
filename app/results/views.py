...
class ResultsView(APIView):
    def get(self, request, job_id, *args, **kwargs):
        ...
        if job_status == "queued":
            return Response(
                {"message": "Your job is in queue"}, status=status.HTTP_202_ACCEPTED
            )
        ...
        if job_status == "started":
            return Response(
                {"message": "Your job has started"}, status=status.HTTP_202_ACCEPTED
            )

        if job_status == "deferred":
            return Response(
                {"message": "Your job is deferred"}, status=status.HTTP_202_ACCEPTED
            )
