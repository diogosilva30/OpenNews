...
class TestPublicoKeywordsSearch(BaseTestCase):
    ...
    def test_starting_date_older_than_ending_date(self):
        ...
        self.assertEqual(response.json["status"], "error")
        self.assertIn("Invalid dates provided",
                      response.json["message"])

        self.assert400(
            response, "Keyword search should trigger 'Bad Request'! Dates are invalid (starting date > ending date)")
...
class TestPublicoTopicSearch(BaseTestCase):
    ...
    def test_invalid_dates_topic_search_job(self):
        ...
        response = self.client.post("/api/v1/news/publico/topic_search",
                                    json={
                                        "start_date": "not a valid date",
                                        "end_date": "another invalid date",
                                        "keywords": "covid"
