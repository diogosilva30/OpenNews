...
class CMSearch(ABC):
    ...
    login_payload = {'username': os.getenv('CM_USER'),
...
class CMTopicSearch(CMSearch):
    ...
    def serialize_to_json(self) -> str:
        ...
        return jsonify({"topic": self.topic,
    ...
    def search(self) -> None:
        ...
        while(True):
            html_string = requests.get(
                f"https://www.cmjornal.pt/mais-sobre/loadmore?friendlyUrl=mais-sobre&urlRefParameters=?ref=Mais%20Sobre_BlocoMaisSobre&contentStartIndex={index}&searchKeywords={self.topic.replace(' ', '-')}").text
